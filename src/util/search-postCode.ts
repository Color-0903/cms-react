...
const _fetch = (url: string, yubin7: any) => {
  fetch(url)
...
const _setAddr = (text: string, yubin7: any) => {
  ...
  if (matcher) {
    ...
    if (addr && addr[0] && addr[1]) {
      ...
      const location = {
        region_id: addr[0],
        region: PREFECTURES[index]['label'],
        locality: addr[1],
        street: addr[2],
        extended: addr[3],
      };
...
const _getAddr = (yubin7: any) => {
  const yubin3 = yubin7.substr(0, 3);
  return new Promise((resolve, reject) => {
    fetch(`${URL}/${yubin3}.js`)
      .then((response) => response.text())
      .then((text) => {
        resolve(_setAddr(text, yubin7));
      })
      .catch((error) => {
        reject(error);
      });
...
export const searchPostalCode = (inputValue: string) => {
  return new Promise((resolve, reject) => {
    if (inputValue) {
