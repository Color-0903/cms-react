...
import { assetsApi, docApi } from '../../../../apis';
import { CreateDocsDto, UpdateDocsDto } from '../../../../apis/client-axios';
...
import { QUERY_DOCUMENT_DETAIL, QUERY_LIST_NEWS } from '../../../../util/contanst';
...
const DocAction = () => {
  ...
  const { data: dataDoc, isFetching: loadingData } = useQuery(
    [QUERY_DOCUMENT_DETAIL, id],
    () => docApi.docsControllerGetDetail(id as string),
    {
      onError: (error) => {},
      onSuccess: (response) => {
  ...
  const {
    mutate: DeleteDoc,
    status: statusDeleteDoc,
    isLoading: deleteLoading,
  } = useMutation((id: string) => docApi.docsControllerDeleteDocs(id), {
    onSuccess: (data) => {
  ...
  const {
    mutate: docCreate,
    status: statusCreateDoc,
    isLoading: createLoading,
  } = useMutation((createDoc: CreateDocsDto) => docApi.docsControllerCreateDocs(createDoc), {
    onSuccess: ({ data }) => {
      navigate(`/admin/${ADMIN_ROUTE_NAME.HELP_MANAGEMENT}`);
  ...
  const {
    mutate: docUpdate,
    status: statusUpdateDoc,
    isLoading: updateLoading,
  } = useMutation((updateDoc: UpdateDocsDto) => docApi.docsControllerUpdateDocs(id as string, updateDoc), {
    onSuccess: ({ data }) => {
      navigate(`/admin/${ADMIN_ROUTE_NAME.HELP_MANAGEMENT}`);
  ...
  const customRequest = async (options: any) => {
    ...
    setLoadingImg(true);
    UploadImage({ file, assetFolderId: undefined, s3FilePath: 'docs' });
  };
  ...
  const uploadFunction = (blobInfo: any, progress: any) =>
    new Promise((resolve, reject) => {
      ...
      formData.append('s3FilePath', 'docs');
  ...
  return (
    <Card id="create-doc-management">
      ...
      >
        <div className="container-doc">
          ...
          >
            ...
            <Editor
              ...
              onEditorChange={(docValue, editor) => {
                handleChangeEditor(docValue, editor);
              }}
            />
            {isSubmit && dataEditor === '' && (
              <span className="text-error">
                {intl.formatMessage({
                  id: 'doc.create.error.content',
                })}
              </span>
            )}
          </div>
            ...
            <div className="right-container__action">
              {id ? (
                <div className="more-action">
                  ...
                  >
                    {intl.formatMessage({
                      id: 'doc.edit.button.save',
                    })}
                  </CustomButton>
                  <CustomButton
                    className="button-delete"
                    onClick={() => {
                      permission.delete && setIsDeleteDoc(true);
                    }}
                    disabled={!permission.delete}
                  >
                    {intl.formatMessage({
                      id: 'doc.edit.button.delete',
                    })}
                  </CustomButton>
                </div>
              ) : (
        ...
        <ConfirmDeleteModal
          name={dataDoc?.data.title || ''}
          visible={isDeleteDoc}
          onSubmit={handleDelete}
          onClose={() => setIsDeleteDoc(false)}
        />
      </Spin>
    </Card>
  );
