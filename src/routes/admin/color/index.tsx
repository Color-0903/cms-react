...
import { colorApi } from '../../../apis';
import { CreateColorDto, UpdateColorDto } from '../../../apis/client-axios';
...
const ColorList = () => {
  ...
  const { data: _data, isLoading: _isLoading } = useQuery({
    ...
    queryFn: () => colorApi.colorControllerGetById(isSelect?.id as string),
    onSuccess: ({ data }) => {
  ...
  const { mutate: Update, isLoading: isUpdate } = useMutation(
    (dto: UpdateColorDto) => colorApi.colorControllerUpdate(isSelect?.id as string, dto),
    {
      onSuccess: (data: any) => {
        setIsOpen(false);
  ...
  const { mutate: Create, isLoading: isCreate } = useMutation(
    (dto: CreateColorDto) => colorApi.colorControllerCreate(dto),
    {
      onSuccess: (data: any) => {
        setIsOpen(false);
        queryClient.invalidateQueries([QUERY_LIST_CATEGPRY]);
  ...
  const { mutate: Delete, isLoading: isDelete } = useMutation((id: string) => colorApi.colorControllerDelete(id), {
    onSuccess: (data: any) => {
      setIsOpen(false);
      queryClient.invalidateQueries([QUERY_LIST_CATEGPRY]);
  ...
  return (
    <Spin spinning={isCreate || isUpdate || isLoading || (isOpen && _isLoading)}>
      ...
      <CreateModal alias="color" isOpen={isOpen} onSubmit={onSubmit} onClose={onClose} form={form} />
